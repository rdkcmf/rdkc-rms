...
#########################################################################
#
# test rms mp4 input, rtmp streaming, and hds output via telnet commands
# 2012-may-14
#
# requires:  ruby 1.8.7
...
#
# syntax:    test-hds             # uses ..\media\bunny.mp4 as default input
#            test-hds <name>      # uses ..\media\<name>.mp4 as custom input
...
# main body of script
# tests streaming of input mp4 file to output hds files
...
# - uses rms createhdsstream to convert rtmp stream to hds files
# - requires osmf player to playback hds video
begin
	...
	# b. set input filename and output hds folder
	...
	# a. prepare to convert incoming rtmp stream to local stream
	puts
	...
	# b. delay before createhdsstream
	sleep 3

	# 7. telnet command: createhdsstream
	# convert local stream to hds
	puts
	(status, details) = command(localhost, "createhdsstream localstreamnames=#{source}11 \
			targetfolder=#{WEBDIR}/#{TARGET} groupname=#{source}11g rolling=1")
	raise "error on createhdsstream" if (status != "SUCCESS")

	# 8. telnet command: liststreams
	# list active streams until hds playlist detected or max loops reached
	20.times do |n|
	...
	# 9. confirm if hds playlist exists
	puts
	# a. check playlist
	...
	# b. delay at least 3 fragments before playing
	sleep 30

	# 10. use osmf player and the link below to playback hds video
	puts
	puts "OSMF MEDIA URL: http://localhost/#{TARGET}/#{source}11g/#{PLAYLIST}"
