...
class Usage
  ...
  def getMemUsages
    ...
    @pids.size.times do |i|
      usage = `cat /proc/#{@pids[i]}/statm 2> /dev/null`.split
  ...
  def getCpuUsages
    ...
    usage = `head -1 /proc/stat 2> /dev/null`.split
    cpuTotalTicks = 0
    ...
    pidUsages = []
    @pids.size.times do |i|
      usage = `cat /proc/#{@pids[i]}/stat 2> /dev/null`.split
      utime = usage[13].to_i
      stime = usage[14].to_i
  ...
  def saveStatistics(fileName, elapsedSec, streamCount, cpuUsages, memUsages, bwUsages, port)
    ...
    if (elapsedSec == 0)
      ...
      file.puts "sec, count, #{cpuHeader}#{memHeader}#{bwHeader}#{port}"
    end
