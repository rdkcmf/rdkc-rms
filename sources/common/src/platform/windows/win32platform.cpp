...
int vasprintf(char **strp, const char *fmt, va_list ap) {
	...
	if (size >= 0 && size < INT_MAX) {
		*strp = (char*)malloc(size + 1);
		if (NULL != *strp) {
			result = vsnprintf(*strp, size + 1, fmt, ap);
...
bool setFdTTL(SOCKET fd, uint8_t ttl) {
	// this is char since unsigned char(uint8_t) is incompatible with setsockopt in Win32 w/o casting
	char temp = ttl;
	// Set TTL based on passed TTL pushStream parameter
	if (setsockopt(fd, IPPROTO_IP, IP_TTL, &temp, sizeof (ttl)) != 0) {
		int err = errno;
...
bool setFdMulticastTTL(SOCKET fd, uint8_t ttl) {
	// this is char since unsigned char(uint8_t) is incompatible with setsockopt in Win32 w/o casting
	char temp = ttl;
	// Set TTL based on passed TTL pushStream parameter for multicast UDP
	if (setsockopt(fd, IPPROTO_IP, IP_MULTICAST_TTL, &temp, sizeof (ttl)) != 0) {
		int err = errno;
...
bool listFolder(string path, vector<string> &result, bool normalizeAllPaths,
	...
	// also append '\' to the path for normalization later.
...
bool copyFile(string src, string dst, bool overwrite) {
	if (!CopyFile(STR(src), STR(dst), !overwrite)) {
		FATAL("Unable to copy file `%s` to `%s`",
				STR(src), STR(dst));
		return false;
	}
	return true;
...
void GetFinishedProcesses(vector<pid_t> &pids, bool &noMorePids) {
	...
	while (truePidCount) {
		...
		if (resp == WAIT_TIMEOUT) { // break when there is no terminated processes 
			for (uint32_t handle = 0; handle < pidCount; ++handle) {
				CloseHandle(hProcs[handle]);
			}
			delete[] hProcs;
			break;
		}
		...
		if (resp == WAIT_FAILED) {
			...
			HANDLE hProcess = hProcs[0]; //Set this to zero since we are moving the mem block always to the
										 //first element, setting this to resp will cause a crash at some point
		...
		truePidCount--;
		//same as above resp should not be used in indexing hProcs, since resp can get a value higher
		//than the upper bound of hProcs which can cause a crash. Also the size should be pidCount - 1
		//else the memory can get corrupted as we will be moving memory not belonging to the program
		memmove(&hProcs[0], &hProcs[1], (pidCount - 1) * sizeof(HANDLE));
	};
}
...
bool LaunchProcess(string fullBinaryPath, vector<string> &arguments, vector<string> &envVars, pid_t &pid) {
	...
	if (gotParentsVar == false) {
		...
		//TODO: what if the client updated the environment and RMS was already called with launchProcess?
		gotParentsVar = true;
	}
...
void InstallCrashDumpHandler(const char *pPrefix) {
	//TODO: Implement windows version of the crashdump
}
