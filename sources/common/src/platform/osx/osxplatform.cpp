...
bool setFdJoinMulticast(SOCKET sock, string bindIp, uint16_t bindPort, string ssmIp, uint16_t sockFamily) {
	...
	if ( sockFamily == AF_INET) {
		...
		if (ssmIp == "") {
			...
			if (setsockopt(sock, IPPROTO_IPV6, IPV6_JOIN_GROUP /*IPV6_ADD_MEMBERSHIP*/,
					(char *) &group, sizeof (group)) < 0) {
				int err = errno;
				FATAL("Adding multicast failed. Error was: (%d) %s", err, strerror(err));
				return false;
...
#ifdef IOS
	// Added MSG_NOSIGNAL to send() instead
#else
    int32_t one = 1;
...
bool DetermineMaxRcvSndBuff(int option, bool isUdp) {
	...
	__maxSndBufSocket = -1;
	// See https://developer.apple.com/library/content/documentation/NetworkingInternetWeb/Conceptual/NetworkingOverview/CommonPitfalls/CommonPitfalls.html
	//__maxSndBufSocket = socket(PF_INET, isUdp ? SOCK_DGRAM : SOCK_STREAM, isUdp ? IPPROTO_UDP : IPPROTO_TCP);
...
string getHostByName(string name) {
    ...
    for(currAddr = servinfo; currAddr != NULL; currAddr = currAddr->ai_next) {
        void *networkAdressSrc;

        switch (currAddr->ai_family) {
        /* IPv4 family */
        case AF_INET:
			struct sockaddr_in *sin = (struct sockaddr_in *)currAddr->ai_addr;
			networkAdressSrc = &(sin->sin_addr);
			break;

        /* IPv6 family */
        default:
			struct sockaddr_in6 *sin6 = (struct sockaddr_in6 *)currAddr->ai_addr;
			networkAdressSrc = &(sin6->sin6_addr);
			break;
        }

        /* IP addresses from binary to string */
        inet_ntop(currAddr->ai_family, networkAdressSrc, ipstr, sizeof (ipstr));
		result = ipstr;
...
*******************************************************/
    ...
    for(p = servinfo; p != NULL; p = p->ai_next) {
	    ...
	    continue; // skip IPV6 first to force IPV4 on the top
        }
    ...
    for(p = servinfo; p != NULL; p = p->ai_next) {
        ...
        if (p->ai_family == AF_INET) { // IPv4
	    continue; // skip IPV4 as its already been added
        } else { // IPv6
...
bool copyFile(string src, string dst, bool overwrite) {
	...
	if (!overwrite)
		flags |= COPYFILE_EXCL;
    bool result = (copyfile(STR(src), STR(dst), s, flags) >= 0);
