...
#ifdef VARIANT_PARSE_ASSERT
#define PARSE_FAIL ASSERT
#else
#define PARSE_FAIL FATAL
#endif
...
Variant::operator bool() {
	switch (_type) {
		...
		default:
		{
			PARSE_FAIL("Cast to bool failed: %s", STR(ToString()));
			return false;
		}
	}
}
...
Variant::operator ctype() {\
    switch (_type) {\
        ...
        default:\
        {\
            PARSE_FAIL("Cast failed: %s", STR(ToString()));\
            return 0;\
        }\
    }\
}
...
Variant::operator Timestamp() {
	if ((_type == V_DATE)
		...
		PARSE_FAIL("Cast to struct tm failed: %s", STR(ToString()));
...
Variant::operator string() {
	switch (_type) {
		...
		default:
		{
			PARSE_FAIL("Cast to string failed: %s", STR(ToString()));
			return "";
		}
	}
}
...
bool Variant::operator==(const Variant &value) const {
	...
	switch (_type) {
		...
		default:
		{
			PARSE_FAIL("Invalid variant type: %d", _type);
			return false;
		}
	}
}
...
string Variant::GetTypeName() {
	if (_type != V_TYPED_MAP) {
		PARSE_FAIL("GetMapName failed: %s", STR(ToString()));
		return "";
	}
...
void Variant::SetTypeName(string name) {
	if ((_type != V_TYPED_MAP)
			...
			&& (_type != V_UNDEFINED)
			&& (_type != V_NULL)) {
		PARSE_FAIL("SetMapName failed: %s", STR(ToString()));
		return;
	}
...
void Variant::RemoveKey(const string &key, bool caseSensitive) {
	if ((_type != V_TYPED_MAP) && (_type != V_MAP)) {
		PARSE_FAIL("RemoveKey failed: %s", STR(ToString()));
		return;
	}
	if (caseSensitive) {
...
void Variant::RemoveAllKeys() {
	if ((_type != V_TYPED_MAP) && (_type != V_MAP)) {
		PARSE_FAIL("RemoveAllKeys failed: %s", STR(ToString()));
		return;
	}
...
uint32_t Variant::MapSize() {
	...
	if ((_type != V_TYPED_MAP) && (_type != V_MAP)) {
		PARSE_FAIL("MapSize failed: %s", STR(ToString()));
		return 0;
	}
...
bool Variant::SerializeToXmlFile(string fileName) {
	...
	File file;
	if (!file.Initialize(fileName, FILE_OPEN_MODE_TRUNCATE)) {
		WARN("Unable to open file %s", STR(fileName));
		return false;
	}
