...
bool DHWrapper::Initialize() {
	...
	DH_set0_pqg(_pDH, p, NULL, g);//transfers p and g memory management to _pDH
	if (DH_generate_key(_pDH) != 1) {
		FATAL("Unable to generate DH public/private keys");
		Cleanup();
		return false;
...
bool DHWrapper::CopyPublicKey(uint8_t *pDst, int32_t dstLength) {
	...
	#else
	BIGNUM *pub_key;
	DH_get0_key(_pDH, (const BIGNUM**)&pub_key, NULL);
...
bool DHWrapper::CopyPrivateKey(uint8_t *pDst, int32_t dstLength) {
	...
	#else
	BIGNUM *priv_key;
	DH_get0_key(_pDH, NULL, (const BIGNUM**)&priv_key);
...
void HMACsha1(const void *pData, uint32_t dataLength,
		const void *pKey, uint32_t keyLength, void *pResult) {
	unsigned int digestLen;
	...
	HMAC_Init_ex(ctx, (unsigned char*) pKey, keyLength, EVP_sha1(), NULL);
	HMAC_Update(ctx, (unsigned char *) pData, dataLength);
	HMAC_Final(ctx, (unsigned char *) pResult, &digestLen);

	HMAC_CTX_cleanup(ctx);
	//FATAL("$b2$ HMACsha1 digestLen = %d", digestLen);
	// I cloked this at 20, which is what I want!
	//o_assert(digestLen == 32);
	#else
	HMAC_CTX* ctx = HMAC_CTX_new();
...
uint32_t calcCrc32(const uint8_t *p, uint32_t size) {
	uint32_t crc = ~0U;
