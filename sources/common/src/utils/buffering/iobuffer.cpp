...
bool IOBuffer::ReadFromUDPFd(int32_t fd, int32_t &recvAmount, SocketAddress &peerAddress) {
	SANITY_INPUT_BUFFER;
	...
	_dummy = peerAddress.getLength();
	recvAmount = recvfrom(fd, (char *) (_pBuffer + _published), 65536,
			MSG_NOSIGNAL, peerAddress.getSockAddr(), &_dummy);
	if (recvAmount > 0) {
		WRITE_IN_WITNESS(_pBuffer + _published, recvAmount);
		_published += (uint32_t) recvAmount;
		SANITY_INPUT_BUFFER;
...
bool IOBuffer::Ignore(uint32_t size) {
	SANITY_INPUT_BUFFER;
	uint32_t availableSize = _published - _consumed;
	if (availableSize < size) {
		FATAL("Size to be ignored is greater than available size.");
		return false;
	}
	_consumed += size;
	if (_sendLimit != 0xffffffff)
...
string IOBuffer::DumpBuffer(MSGHDR &message, uint32_t limit) {
	IOBuffer temp;
	for (MSGHDR_MSG_IOVLEN_TYPE i = 0; i < (uint32_t)message.MSGHDR_MSG_IOVLEN; i++) {
