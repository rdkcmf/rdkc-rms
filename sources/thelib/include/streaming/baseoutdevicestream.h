...
#ifndef _BASEOUTDEVICESTREAM_H
#define	_BASEOUTDEVICESTREAM_H
...
class BaseOutDeviceStream
: public BaseOutStream {
private:

	class OutDeviceStreamTimer
	: public BaseTimerProtocol {
	private:
		BaseOutDeviceStream *_pOutDeviceStream;
	public:
		OutDeviceStreamTimer(BaseOutDeviceStream *pOutDeviceStream);
		virtual ~OutDeviceStreamTimer();
		void ResetStream();
		virtual bool TimePeriodElapsed();
	};
	friend class OutDeviceStreamTimer;
...
protected:
	...
	bool _hasAudio;
public:
	BaseOutDeviceStream(BaseProtocol *pProtocol, uint64_t type, string name);
	virtual ~BaseOutDeviceStream();
