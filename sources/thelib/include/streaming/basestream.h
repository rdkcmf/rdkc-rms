...
typedef LinkedList<uint32_t, BaseOutStream *> StreamsList;
typedef LinkedListNode<uint32_t, BaseOutStream *> StreamNode;
...
protected:
	...
	struct BaseStreamStats {

		struct BaseStreamTrackStats {
			uint64_t bytesCount;
...
public:
	...
	/* SavePts(double pts) should be called by all InStreams within FeedData()
	 * added 4/9/15, to support timestamps in metadata
	 */
	virtual void SavePts(double pts) {_pts = pts;};
	virtual double GetPts() { return _pts;};
	virtual int64_t GetPtsInt64()  { return (int64_t)_pts;};
protected:
	double _pts;
public:
	/*!
	...
	/*!
	 * @brief SendMetadata behaves differently on instreams and outsreams
	 * for an instream it will pass to all its outstreams -
	 * only a select few outStreams will implement this call.
	 * An outStream implementing this will send it somehow over its protocol.
	 */
	virtual bool SendMetadata(string metadataStr) { NYIR; };  // Over-ride me!
	virtual bool SendMetadata(string const& metadataStr, int64_t pts) { NYIR; };  // Over-ride me!
