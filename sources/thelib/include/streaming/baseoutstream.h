...
private:
    ...
    double _maxWaitDts;
    bool _nalSEIExists;
    Variant _videoParamsSizes;
protected:
    BaseInStream *_pInStream;
	bool _hasAudio;
	string _origOutStreamName;
	BaseInStream *_pOrigInStream;
public:
	...
	// it is advisable to call this every time there's a need to access for orig instream,
	// to avoid accessing an already deleted/freed instream pointer
	virtual BaseStream* GetOrigInstream(); 
protected:
    /*!
    ...
    virtual bool PushVideoData(IOBuffer &/*buffer*/, double /*pts*/, double /*dts*/, Variant& /*videoParams*/) { return false; }
	...
	/*!
	* @brief This function will be called by the FeedMetaData function
	* when a vmf metadata is received
	*
	* @param vmfMetadata - contains the metadata in string format
	*
	* @param pts - presentation timestamp
	*
	* @return should return true for success or false for errors
	*
	* @discussion The receiver of this call should do what is appropriate with
	* the vmfMetadata and return true or false.
	*/
	virtual bool PushMetaData(string const& vmfMetadata, int64_t pts) { return false; }
	
	/**
	 * Determines if this is a case of an H264 variable SPS/PPS.
	 * 
     * @return True if it is, false otherwise.
     */
	bool IsVariableSpsPps(VideoCodecInfo *pOld, VideoCodecInfo *pNew);
private:
...
#ifdef HAS_G711
	bool ProcessG711FromTS(uint8_t *pBuffer, uint32_t length, double pts, double dts);
#endif	/* HAS_G711 */
protected:
	/*
	* Will process vmfMetadata content
	*/
	bool ProcessMetadata(string const& vmfMetadata, int64_t pts);
};
