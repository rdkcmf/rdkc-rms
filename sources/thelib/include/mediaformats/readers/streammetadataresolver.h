...
public:
	...
	VARIANT_GETSET(uint32_t, maxPlaylistFileSize, 0);
};
...
public:
	...
	VARIANT_GETSET(Variant&, playlistElements, _dummy3);
};
...
public:
	 ...
	 * was removed
	 */
	bool RemoveStorage(string mediaFolder, Storage &removedStorage);
	 ...
	 *
	 * @param fileOnly - tells the metadata resolver that the searched stream
	 * is expected to be a file and only a file, not a live stream. That means
	 * that no error messages are issued when the stream name is not found
	 * and this flag is false. If is true, an error message is also printed
	 * This flag doesn't influences the return value.
	 *
	 ...
	 */
	bool ResolveMetadata(string streamName, Metadata &result, bool fileOnly);
	...
	/*!
	 * @brief Used by offline HLS/HDS tools to see if I/O is enabled or not
	 */
	bool HasTimers();

	/*!
	 * @brief Used by LSTDocument to get the maximum LST file size
	 */
	uint32_t GetMaxPlaylistFileSize();
private:
	...
	void SetMaxPlaylistFileSize(uint32_t size);
};
