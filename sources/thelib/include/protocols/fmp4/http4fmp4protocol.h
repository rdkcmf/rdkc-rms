...
#ifndef _HTTP4FMP4PROTOCOL_H
#define	_HTTP4FMP4PROTOCOL_H
...
class HTTP4FMP4Protocol
	: public BaseProtocol, HTTPInterface {
private:
	uint32_t _available;
	IOBuffer _outputBuffer;
	uint32_t _returnCode;
	OutNetHLSStream *_pOutStream;
	string _streamName;
	IOBuffer _inputBuffer;
public:
	HTTP4FMP4Protocol();
	~HTTP4FMP4Protocol();
	bool SendRequest(Variant &variant);
	bool SendBlock(uint8_t *pBuffer, uint32_t size);
...
private:
	void SetAck(uint32_t code, string message = "");
	string GetHTTPDescription(uint32_t returnCode);
	//	void UnlinkToHandler();

	bool ProcessFMP4Request();

};

#endif	/* _HTTP4FMP4PROTOCOL_H */
