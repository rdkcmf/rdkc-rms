...
#define STREAM_CAPABILITIES_VERSION MAKE_TAG3('V','1','2')
...
void VideoCodecInfo::GetRTMPMetadata(Variant &destination) {
	...
	double fps = 0;
	if ((fps = GetFPS()) != 0) {
		destination["videoframerate"] = fps;
	}
}
...
bool VideoCodecInfoPassThrough::Serialize(IOBuffer & buffer) {
	if (!VideoCodecInfo::Serialize(buffer)) {
		FATAL("Unable to serialize VideoCodecInfo");
		return false;
	}

	uint32_t temp32 = EHTONL(GETAVAILABLEBYTESCOUNT(_rtmpRepresentation));
	buffer.ReadFromBuffer((uint8_t *) & temp32, sizeof (temp32));

	buffer.ReadFromBuffer(GETIBPOINTER(_rtmpRepresentation),
			GETAVAILABLEBYTESCOUNT(_rtmpRepresentation));

	return true;
}

bool VideoCodecInfoPassThrough::Deserialize(IOBuffer & buffer) {
	...
	if (length < sizeof (uint32_t)) {
		FATAL("Not enough data to deserialize VideoCodecInfoPassThrough");
		return false;
	}
	uint32_t temp = ENTOHLP(pBuffer);
	if (length < sizeof (uint32_t) + temp) {
		FATAL("Not enough data to deserialize VideoCodecInfoPassThrough");
		return false;
	}

	_rtmpRepresentation.ReadFromBuffer(pBuffer + sizeof (uint32_t), temp);
	return buffer.Ignore(sizeof (uint32_t) + temp);
}
...
IOBuffer & VideoCodecInfoPassThrough::GetRTMPRepresentation() {
	return _rtmpRepresentation;
}

void VideoCodecInfoPassThrough::SetRTMPRepresentation(const uint8_t *pBuffer, uint32_t length) {
	_rtmpRepresentation.IgnoreAll();
	_rtmpRepresentation.ReadFromBuffer(pBuffer, length);
}
...
bool VideoCodecInfoH264::Init(uint8_t *pSPS, uint32_t spsLength, uint8_t *pPPS,
		uint32_t ppsLength, uint32_t samplingRate, int16_t a, int16_t b, bool obfuscate) {
	if ((spsLength < 8)
			|| (spsLength > 65535)
			|| (ppsLength == 0)
			|| (ppsLength > 65535)) {
...
#define ENFORCE_LICENSE
	...
	if ((License::GetLicenseInstance()->GetFlags() & LICENSE_FLAGS_SPS_OBFUSCATED) != 0) {
		if (obfuscate) {
			if ((a < 0) || (b < 0)) {
				a = 0;
				b = 4;
...
bool AudioCodecInfoPassThrough::Serialize(IOBuffer & buffer) {
...
bool AudioCodecInfoPassThrough::Deserialize(IOBuffer & buffer) {
	...
	if (length < sizeof (uint32_t)) {
		FATAL("Not enough data to deserialize AudioCodecInfoPassThrough");
		return false;
	}
	uint32_t temp = ENTOHLP(pBuffer);
	if (length < sizeof (uint32_t) + temp) {
...
IOBuffer & AudioCodecInfoPassThrough::GetRTMPRepresentation() {
	return _rtmpRepresentation;
}

void AudioCodecInfoPassThrough::SetRTMPRepresentation(const uint8_t *pBuffer, uint32_t length) {
	_rtmpRepresentation.IgnoreAll();
	_rtmpRepresentation.ReadFromBuffer(pBuffer, length);
}
...
#ifdef HAS_G711
AudioCodecInfoG711::AudioCodecInfoG711()
: AudioCodecInfo() {
}
...
bool AudioCodecInfoG711::Init(bool aLaw) {
	_type = aLaw ? CODEC_AUDIO_G711A : CODEC_AUDIO_G711U;
	_samplingRate = 8000;
	_channelsCount = 1;
	_bitsPerSample = 8;
	return true;
}
bool AudioCodecInfoG711::Serialize(IOBuffer & buffer) {
	if (!AudioCodecInfo::Serialize(buffer)) {
		FATAL("Unable to serialize AudioCodecInfo");
		return false;
	}
...
bool AudioCodecInfoG711::Deserialize(IOBuffer & buffer) {
	if (!AudioCodecInfo::Deserialize(buffer)) {
		FATAL("Unable to deserialize AudioCodecInfo");
		return false;
	}
...
//TODO
IOBuffer & AudioCodecInfoG711::GetRTMPRepresentation() {
	if (GETAVAILABLEBYTESCOUNT(_rtmpRepresentation) != 0)
		return _rtmpRepresentation;
	uint8_t headerByte = 0;
	if (_type == CODEC_AUDIO_G711U)
		headerByte |= (RTMP_AUDIO_FORMAT_PCMU << 4);
	else
		headerByte |= (RTMP_AUDIO_FORMAT_PCMA << 4);

	// fitting into the nearest smaple rate enumerated
	// using geometric mean between enums;
	if (_samplingRate > 31113)	// greater than geometric mean of 22k and 44k
		headerByte |= (RTMP_AUDIO_RATE_44000 << 2);
	else if (_samplingRate > 15556)
		headerByte |= (RTMP_AUDIO_RATE_22000 << 2);
	else if (_samplingRate > 7778)
		headerByte |= (RTMP_AUDIO_RATE_11000 << 2);
	else
		headerByte |= (RTMP_AUDIO_RATE_5500 << 2);

	if (_bitsPerSample == 8)
		headerByte |= (RTMP_AUDIO_SIZE_8 << 1);
	else
		headerByte |= (RTMP_AUDIO_SIZE_8 << 1);

	if (_channelsCount == 1)
		headerByte |= RTMP_AUDIO_TYPE_MONO;
	else
		headerByte |= RTMP_AUDIO_TYPE_STEREO;
...
void AudioCodecInfoAAC::GetRTMPMetadata(Variant &destination) {
	AudioCodecInfo::GetRTMPMetadata(destination);
	destination["aacaot"] = _audioObjectType;
}
...
VideoCodecInfoH264* StreamCapabilities::AddTrackVideoH264(uint8_t *pSPS,
	...
	if ((_pVideoTrack != NULL)
            ...
            // just carry over the current sps/pps, disable force cut when the current 
            // sps/pps value is not equal to previous sps/pps. This is a fix/workaround to 
            // issue with Hughes Ericsson encoder. This encoder has a changing PPS throughout
            // the stream and RMS can't handle it.
	...
	if (!pCodecInfo->Init(pSPS, spsLength, pPPS, ppsLength, samplingRate, a, b,
			obfuscate)) {
...
AudioCodecInfoMP3 * StreamCapabilities::AddTrackAudioMP3(uint32_t samplingRate,
	...
	if (!pCodecInfo->Init(samplingRate, channelsCount, bitsPerSample)) {
		FATAL("Unable to initialize AudioCodecInfoMP3");
		delete pCodecInfo;
		return NULL;
	}
	AudioCodecInfo *pOld = _pAudioTrack;
...
void StreamCapabilities::FalseAudioCodecUpdate(BaseInStream *pInStream) {
	AudioCodecInfoPassThrough *pCodecInfo = new AudioCodecInfoPassThrough();
	_pAudioTrack = pCodecInfo;

	if (pInStream != NULL)
		pInStream->AudioStreamCapabilitiesChanged(this, NULL, pCodecInfo);
}

void StreamCapabilities::FalseVideoCodecUpdate(BaseInStream *pInStream) {
	VideoCodecInfoPassThrough *pCodecInfo = new VideoCodecInfoPassThrough();
	_pVideoTrack = pCodecInfo;

	if (pInStream != NULL)
		pInStream->VideoStreamCapabilitiesChanged(this, NULL, pCodecInfo);
}

#ifdef HAS_G711
AudioCodecInfoG711 * StreamCapabilities::AddTrackAudioG711(bool aLaw, BaseInStream *pInStream) {
	if ((_pAudioTrack != NULL)
			&&(_pAudioTrack->_type == CODEC_AUDIO_G711A)) {
		return (AudioCodecInfoG711*) _pAudioTrack;
	}
	AudioCodecInfoG711 *pCodecInfo = new AudioCodecInfoG711();
	if (!pCodecInfo->Init(aLaw)) {
		FATAL("Unable to initialize AudioCodecInfoG711");
		delete pCodecInfo;
		return NULL;
	}
	AudioCodecInfo *pOld = _pAudioTrack;
