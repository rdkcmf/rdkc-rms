...
bool StreamMetadataResolver::Initialize(Variant &configuration) {
	...
	if (configuration.HasKeyChain(V_BOOL, false, 1, "hasTimers")) {
		_hasTimers = (bool)configuration.GetValue("hasTimers", false);
...
bool StreamMetadataResolver::InitializeStorage(string name, Variant &config,
	...
	//maxPlaylistFileSize
	uint32_t maxPlaylistFileSize = 0;
	if (config.HasKeyChain(_V_NUMERIC, false, 1, "maxPlaylistFileSize")) {
		maxPlaylistFileSize = (uint32_t) config.GetValue("maxPlaylistFileSize", false);
	}
	if (maxPlaylistFileSize < MIN_LST_FILE_LIMIT) {
		maxPlaylistFileSize = MIN_LST_FILE_LIMIT;
	} else if (maxPlaylistFileSize > MAX_LST_FILE_LIMIT) {
		maxPlaylistFileSize = MAX_LST_FILE_LIMIT;
	}
	//INFO("maxPlaylistFileSize = %"PRIu32, maxPlaylistFileSize);
	...
	pStorage->maxPlaylistFileSize(maxPlaylistFileSize);
...
bool StreamMetadataResolver::RemoveStorage(string mediaFolder, Storage &removedStorage) {
...
bool StreamMetadataResolver::ResolveMetadata(string streamName, Metadata &result,
	...
	if (!ResolveStorage(result)) {
		if (fileOnly)
			FATAL("Stream name %s not found in any storage", STR(streamName));
		return false;
	}
	
	//3. Compute seek/meta file paths
	if (!ComputeSeekMetaPaths(result)) {
		FATAL("Unable to compute seek/meta file paths for %s", STR(streamName));
		return false;
	}
	
	//4. Compute seek/meta files
	if (!ComputeSeekMeta(result)) {
		FATAL("Unable to compute seek/meta files %s", STR(streamName));
		return false;
	}
...
bool StreamMetadataResolver::ResolveStreamName(string streamName, Metadata &result) {
	...
	if ((type == MEDIA_TYPE_MOV)
		...
		type = MEDIA_TYPE_FLV;
		supported = true;
	} else if (type == MEDIA_TYPE_VOD) {
		type = MEDIA_TYPE_VOD;
		supported = true;
	} else if (type == MEDIA_TYPE_LST) {
		type = MEDIA_TYPE_LST;
		supported = true;
	}
...
void StreamMetadataResolver::GenerateMetaFiles() {
	...
	FOR_VECTOR(_storagesByOrder, i) {
		...
		FOR_VECTOR(files, j) {
			temp.Reset();
			if (!ResolveMetadata(files[j], temp, true)) {
				WARN("Unable to generate metadata for file %s", STR(files[j]));
			}
		}
	}
}
...
bool StreamMetadataResolver::HasTimers() {
	return _hasTimers;
}

uint32_t StreamMetadataResolver::GetMaxPlaylistFileSize() {
	return _maxPlaylistFileSize;
}
...
bool StreamMetadataResolver::ComputeSeekMetaPaths(Metadata &result) {
	...
	if (storage.metaFolder() == storage.mediaFolder()) {
		...
		if (!testFile.Initialize(storage.metaFolder() + result.hash(), FILE_OPEN_MODE_TRUNCATE)) {
			FATAL("folder %s is not write-able by RMS", STR(storage.metaFolder()));
			return false;
		}
		testFile.Close();
...
bool StreamMetadataResolver::ComputeSeekMeta(Metadata &result) {
	...
	if (false) {
		...
		pDocument = new VODDocument(result);
	}
#endif /* HAS_MEDIA_VOD */
#ifdef HAS_MEDIA_LST
	else if (type == MEDIA_TYPE_LST) {
		pDocument = new LSTDocument(result);
	}
#endif /* HAS_MEDIA_LST */

	if (pDocument == NULL) {
		FATAL("Media file type %s not supported", STR(type));
		return false;
	}
...
void StreamMetadataResolver::SetMaxPlaylistFileSize(uint32_t size) {
	if (size < MIN_LST_FILE_LIMIT) {
		size = MIN_LST_FILE_LIMIT;
	} else if (size > MAX_LST_FILE_LIMIT) {
		size = MAX_LST_FILE_LIMIT;
	}
	_maxPlaylistFileSize = size;
}
