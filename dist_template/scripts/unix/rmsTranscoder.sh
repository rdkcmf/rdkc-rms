...
export AVCONV_DATADIR=__RMS_AVCONV_PRESETS__
...
# Enable file overwrite by default
TRANSCODE="-y"

# Add the extra param
if [ -n "$RMS_RTSP_TRANSPORT" ]; then
  TRANSCODE="$TRANSCODE -rtsp_transport tcp"
fi

# Add -re param as needed
if [ -n "$RMS_FORCE_RE" ]; then
  TRANSCODE="$TRANSCODE -re"
fi
...
TRANSCODE="$TRANSCODE $RMS_CMD_FLAGS -metadata streamName=$RMS_DST_STREAM_NAME"
